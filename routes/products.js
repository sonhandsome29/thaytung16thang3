...
let { getRedisClient, isRedisReady } = require('../utils/redisClient');
...
async function clearProductCache() {
  ...
  await getRedisClient().del(PRODUCT_CACHE_KEY);
}
...
router.get('/', async function (req, res, next) {
  ...
  try {
    if (isRedisReady()) {
      let cachedProducts = await getRedisClient().get(PRODUCT_CACHE_KEY);
      if (cachedProducts) {
    ...
    if (isRedisReady()) {
      await getRedisClient().setEx(PRODUCT_CACHE_KEY, 60, JSON.stringify(products));
    }
